...
const About = () => {
  ...
  useEffect(() => {
    const fetchAboutData = async () => {
      if (!HAS_BACKEND) {
        setAboutSettings(buildFallbackAboutSettings());
        setTeamMembers(siteContent.team || []);
        setLoading(false);
        return;
      }
  ...
  const socialLinks = useMemo(
    () =>
      [
        ...
        { label: 'WhatsApp', href: aboutSettings?.social?.whatsapp },
        { label: 'Facebook', href: aboutSettings?.social?.facebook },
        { label: 'LinkedIn', href: aboutSettings?.social?.linkedin }
      ]
        .map((item) => ({
          ...item,
          href: normalizeSocialUrl(item.href, item.label.toLowerCase())
        }))
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          {!loadError ? (
            ...
            <p className="text-stone text-sm">Não foi possível carregar o conteúdo.</p>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-white">
      <SeoHelmet
        ...
        image={aboutSettings.cover_image || primaryTeamMember?.image}
      />
      ...
      <div className="bg-porcelain py-28 lg:py-32">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          {teamMembers.length > 0 ? (
            <div className={`grid ${teamGridSizeClass} gap-8 lg:gap-10 mx-auto`}>
              {teamMembers.map((member, index) => (
                <article
                  key={member.id || member.name}
                  className="bg-white border border-gray-200 shadow-premium-sm overflow-hidden animate-slide-up h-full flex flex-col w-full"
                    ...
                    {member.image ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-white">
                        <span className="font-display text-6xl font-bold">
                          {normalizeEditorialText(member.name || 'E').charAt(0)}
                        </span>
                      </div>
                    )}
                  </div>

...
const AdminPostForm = () => {
  ...
  const selectedAuthorMember = useMemo(
    () => teamMembers.find((item) => item.id === formData.author_member_id) || null,
    [teamMembers, formData.author_member_id]
  );
  const previewPost = useMemo(
    () => ({
      ...formData,
      category: selectedCategory?.name || formData.category,
      author_name: selectedAuthorMember?.name || formData.author_name,
      author_image: selectedAuthorMember?.image || formData.author_image
    }),
    [formData, selectedCategory, selectedAuthorMember]
  );
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const [categoriesResponse, postsResponse, teamResponse] = await Promise.all([
          axios.get(apiUrl('/api/categories'), adminFetchConfig()),
          axios.get(apiUrl('/api/posts'), adminFetchConfig()),
          axios.get(apiUrl('/api/team?published=true'), adminFetchConfig())
        ]);

        setCategories(categoriesResponse.data);
        const nextTeamMembers = Array.isArray(teamResponse.data) ? teamResponse.data : [];
        ...
        if (isEdit) {
          ...
          if (post) {
            ...
            const matchedAuthor =
              nextTeamMembers.find((item) => item.id === post.author_member_id) ||
              nextTeamMembers.find((item) => item.name === post.author_name);
            ...
            setFormData({
              ...
              author_member_id: matchedAuthor?.id || post.author_member_id || '',
              author_name: matchedAuthor?.name || post.author_name || '',
              author_image: matchedAuthor?.image || post.author_image || '',
        ...
        setError('Não foi possível carregar as categorias ou a reportagem.');
      } finally {
  ...
  const handleAuthorMemberChange = (member) => {
    if (!member) {
      setFormData((current) => ({
        ...current,
        author_member_id: '',
        author_image: '',
        author_name: current.author_name || 'Redação Enfoco'
      }));
      return;
    }
  ...
  const handleSubmit = async (event) => {
    ...
    if (!formData.category_id) {
      setError('Selecione uma categoria para a reportagem.');
      return;
    }

    const normalizedContent = normalizeRichTextForStorage(formData.content);
    if (!richTextToPlainText(normalizedContent)) {
      setError('Adicione o conteúdo da reportagem antes de salvar.');
      return;
    }
    ...
    try {
      ...
      const message = saveError?.response?.data?.detail || 'Erro ao salvar reportagem';
      setError(message);
      ...
      <div className="max-w-4xl mx-auto px-4 mt-6">
        <div className="enfoco-glass rounded-[42px] p-8 md:p-10">
          <h1 className="font-display text-4xl font-bold mb-2">
            {isEdit ? 'Editar Reportagem' : 'Nova Reportagem'}
          </h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                {!hasCategories && (
                  <p className="mt-2 text-xs text-stone">
                    Nenhuma categoria encontrada. Crie em <strong>/admin/categories</strong> antes de publicar a reportagem.
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <div className="rounded-[24px] border border-charcoal/10 bg-white/50 p-3 space-y-3">
                  <button
                    type="button"
                    onClick={() => handleAuthorMemberChange(null)}
                    className={`w-full flex items-center gap-3 rounded-2xl border px-3 py-3 text-left transition-colors ${
                      !formData.author_member_id
                        ? 'border-charcoal/30 bg-white'
                        : 'border-charcoal/10 hover:bg-white/70'
                    }`}
                  >
                    <div className="w-10 h-10 rounded-full bg-charcoal text-white flex items-center justify-center text-sm font-bold flex-shrink-0">
                      R
                    </div>
                    <span>
                      <span className="block text-sm font-semibold text-charcoal">Autor manual / Redação</span>
                      <span className="block text-xs text-stone">Use quando o jornalista nao estiver na equipe.</span>
                    </span>
                  </button>
                  ...
                  {teamMembers.map((member) => (
                    <button
                      key={member.id}
                      type="button"
                      onClick={() => handleAuthorMemberChange(member)}
                      className={`w-full flex items-center gap-3 rounded-2xl border px-3 py-3 text-left transition-colors ${
                        formData.author_member_id === member.id
                          ? 'border-royal-blue/50 bg-royal-blue/5'
                          : 'border-charcoal/10 hover:bg-white/70'
                      }`}
                    >
                      {member.image ? (
                        <SafeImage
                          src={member.image}
                          alt={member.name}
                          className="w-12 h-12 rounded-full object-cover object-[center_30%] flex-shrink-0 ring-1 ring-white shadow-sm"
                          loading="lazy"
                          decoding="async"
                          sizes="48px"
                        />
                      ) : (
                        <div className="w-12 h-12 rounded-full bg-charcoal text-white flex items-center justify-center text-sm font-bold flex-shrink-0">
                          {(member.name || 'A').charAt(0)}
                        </div>
                      )}
                      <span>
                        <span className="block text-sm font-semibold text-charcoal">{member.name}</span>
                        <span className="block text-xs text-stone">{member.role || 'Equipe Editorial'}</span>
                      </span>
                    </button>
                  ))}
            ...
            <div className="rounded-[24px] border border-charcoal/10 bg-white/60 px-5 py-5 space-y-4">
              <div>
                ...
                <p className="text-sm text-stone">
                  Escolha se esta reportagem aparece como destaque principal ou secundário na página inicial.
                </p>
              </div>

              {currentFeaturedPost && !formData.destaque_principal_home && (
                <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                  Já existe um destaque principal ativo: <strong>{currentFeaturedPost.title}</strong>. Se você marcar esta reportagem como principal, a atual será desmarcada automaticamente.
                </div>
              )}
              ...
              <div className="grid md:grid-cols-[1fr_1fr_180px] gap-4 items-start">
                <label className="flex items-start gap-3 rounded-2xl border border-charcoal/10 bg-white/70 px-4 py-4">
                  ...
                  <span>
                    ...
                    <span className="block text-xs text-stone mt-1">A home aceita somente uma reportagem nessa posição.</span>
                  </span>
                </label>
                ...
                <label className="flex items-start gap-3 rounded-2xl border border-charcoal/10 bg-white/70 px-4 py-4">
                  ...
                  />
                  <span>
                    <span className="block text-sm font-medium text-charcoal">Destaque secundário</span>
                    <span className="block text-xs text-stone mt-1">Você pode usar vários destaques secundários na home.</span>
                  </span>
                </label>
                ...
                <div>
                  ...
                  <p className="mt-2 text-xs text-stone">
                    {highlightOrderDisabled
                      ? 'Ative o destaque secundário para definir uma ordem.'
                      : 'Usado para ordenar os destaques secundários.'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between gap-4 mb-2">
                <label className="block text-sm font-medium text-charcoal">Conteúdo *</label>
              ...
              {editorView === 'edit' ? (
                <>
                  <RichTextEditor
                    ...
                    placeholder="Estruture a matéria com títulos, subtítulos, parágrafos e links."
                    minHeight={420}
                    token={token}
                  />
                ...
                <div className="space-y-3">
                  <PostPreview post={previewPost} previewLabel="Prévia da matéria" />
                  <p className="text-xs text-stone">
                    Esta visualização usa a mesma tipografia editorial e a mesma hierarquia da matéria publicada no frontend.
                  </p>
                </div>
              )}
            </div>
            ...
            <div>
              <AdminImageField
                ...
                onChange={(nextValue) =>
                  setFormData((current) => ({ ...current, featured_image: nextValue }))
                }
                token={token}
                placeholder="/media/exemplo.jpg"
                positionValue={formData.image_position}
                onPositionChange={(nextValue) =>
                  setFormData((current) => ({ ...current, image_position: nextValue }))
                }
              />
            </div>
            ...
            {editorView === 'edit' && (
              <div className="rounded-[24px] border border-charcoal/10 bg-white/55 px-5 py-5">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                  <div>
                    <p className="text-sm font-medium text-charcoal mb-1">Preview da matéria</p>
                    <p className="text-sm text-stone">
                      Alterne para a aba de prévia para revisar a matéria com o mesmo estilo editorial do site antes de publicar.
                    </p>
                  </div>
                  <button
                    type="button"
            ...
            <div className="flex gap-3 pt-4">
              ...
              >
                {loading ? 'Salvando...' : isEdit ? 'Salvar alterações' : 'Criar reportagem'}
              </button>
            </div>
          </form>
        </div>

...
const AdminPosts = () => {
  ...
  const fetchPosts = async () => {
    try {
      ...
      setNotification({ type: 'error', message: 'Erro ao carregar reportagens.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (postId) => {
    try {
      ...
      setNotification({ type: 'success', message: 'Reportagem deletada com sucesso.' });
      setDeleteConfirm(null);
      fetchPosts();
    } catch (error) {
      console.error('Error deleting post:', error);
      setNotification({ type: 'error', message: 'Erro ao deletar reportagem.' });
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-porcelain via-white to-porcelain">
        <div className="text-center">
          ...
          <p className="text-charcoal font-display text-lg">Carregando reportagens...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 mt-8">
        <div className="enfoco-glass rounded-2xl p-8 md:p-12 mb-10 shadow-premium">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div className="flex-1">
              <h1 className="font-display text-5xl font-bold text-charcoal mb-2">Reportagens</h1>
              <p className="text-stone text-lg font-light">
                Gerenciar reportagens e matérias ({filteredPosts.length})
              </p>
            </div>
        ...
        {filteredPosts.length > 0 ? (
                    ...
                    <div className="flex items-center gap-2 mb-3 flex-wrap">
                      ...
                      {!post.destaque_principal_home && post.destaque_secundario_home && (
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-800 border border-blue-200">
                          Destaque secundário #{Number(post.ordem_destaque || 0)}
                        </span>
                      )}
                      <span
                ...
                {deleteConfirm === post.id && (
                  <div className="mt-4 pt-4 border-t border-charcoal/10 animate-slide-down">
                    <p className="text-sm text-charcoal mb-3">
                      Tem certeza que deseja deletar esta reportagem?
                    </p>
          ...
          <div className="enfoco-glass rounded-xl p-12 text-center shadow-premium">
            ...
            <p className="text-stone mb-6 font-light">Nenhuma reportagem encontrada.</p>
            <Link
              to="/admin/posts/new"

...
const AdminColumnForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchColumn = async () => {
      ...
      try {
        ...
        setError('Não foi possível carregar esta coluna.');
        return null;
      }
    };
  ...
  const handleSubmit = async (event) => {
    ...
    if (!richTextToPlainText(normalizedContent)) {
      setError('Adicione o conteúdo da coluna antes de salvar.');
      return;
    }
        ...
        <div className="enfoco-glass rounded-[42px] p-8 md:p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="rounded-[28px] border border-charcoal/10 bg-white/60 p-5 md:p-6">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setFormData((current) => ({ ...current, columnist_id: event.target.value }))
                  }
                  className="w-full px-4 py-3 rounded-2xl border border-charcoal/10 bg-white/50 focus:outline-none focus:ring-2 focus:ring-charcoal/20"
                >
              ...
              {selectedColumnist ? (
                ...
                <div className="mt-5 rounded-[24px] border border-dashed border-charcoal/12 bg-white/40 p-5">
                  <p className="text-stone text-sm">
                    Sem vinculo: esta coluna continua no modo manual para preservar colunas antigas enquanto você migra os perfis aos poucos.
                  </p>
                </div>
              )}
            </div>
              ...
              <>
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setFormData((current) => ({ ...current, author_name: event.target.value }))
                      }
                      className="w-full px-4 py-3 rounded-2xl border border-charcoal/10 bg-white/50 focus:outline-none focus:ring-2 focus:ring-charcoal/20"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setFormData((current) => ({ ...current, author_role: event.target.value }))
                      }
                      className="w-full px-4 py-3 rounded-2xl border border-charcoal/10 bg-white/50 focus:outline-none focus:ring-2 focus:ring-charcoal/20"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      setFormData((current) => ({ ...current, author_bio: event.target.value }))
                    }
                ...
                <AdminImageField
                  ...
                  onChange={(nextValue) =>
                    setFormData((current) => ({ ...current, author_image: nextValue }))
                  }
                  token={token}
              ...
              {editorView === 'edit' ? (
                <>
                  ...
                  <p className="mt-3 text-xs text-stone">
                    Este editor segue o mesmo padrão de Reportagens, com estrutura, tipografia, espaçamento, imagens e galerias no meio do conteúdo.
                  </p>
                </>
              ) : (
                <div className="space-y-3">
                  <ColumnPreview column={previewColumn} previewLabel="Prévia da coluna" />
                  <p className="text-xs text-stone">
                    Esta visualização usa a mesma hierarquia editorial da coluna publicada no frontend.
                  </p>
                </div>
              )}
            </div>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm text-stone">
                      Alterne para a aba de prévia para revisar a coluna com o mesmo estilo do frontend antes de publicar.
                    </p>
                  </div>
                  <button
                    type="button"
            ...
            <div className="flex gap-3 pt-4">
              ...
              >
                {loading ? 'Salvando...' : isEdit ? 'Salvar Alterações' : 'Criar Coluna'}
              </button>
            </div>
          </form>
        </div>

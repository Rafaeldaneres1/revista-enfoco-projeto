...
const buildRecentActivity = ({ posts, columns, events, editions }) => {
  const allItems = [
    ...posts.map((item) => ({
      title: item.title,
      description: `Reportagem ${item.published ? 'publicada' : 'em rascunho'}`,
...
const AdminDashboard = () => {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setImportMessage(
        `✓ Conteúdo sincronizado: ${result.posts.created + result.posts.updated} reportagens, ${result.columns.created + result.columns.updated} colunas, ${result.editions.created + result.editions.updated} edições, ${result.team.created + result.team.updated} perfis da equipe.`
      );
  ...
  const contentCards = [
    {
      title: 'Reportagens',
      description: 'Gerenciar reportagens e matérias',
      to: '/admin/posts',
      icon: '📰',
    ...
    {
      title: 'Comentários',
      description: stats.pendingComments > 0
        ? `${stats.pendingComments} aguardando aprovação`
  ...
  settingsCards.push({
    title: 'LGPD',
    description: 'Configurar privacidade, cookies e pixels',
    to: '/admin/privacy',
    icon: 'LGPD'
  });
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
              <div className="flex-1">
                ...
                <p className="text-stone font-light">
                  Importa para o painel as reportagens, colunas, edições e perfis da equipe que já foram montados no site, para que tudo fique editável no admin.
                </p>
              </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-stone font-medium">Crie sua primeira reportagem</p>

...
const ALLOWED_TAGS = new Set(['P', 'BR', 'STRONG', 'B', 'EM', 'I', 'U', 'H1', 'H2', 'H3', 'UL', 'OL', 'LI', 'A', 'SPAN', 'IMG', 'FIGURE', 'FIGCAPTION']);
const BLOCK_TAGS = new Set(['P', 'H1', 'H2', 'H3', 'UL', 'OL', 'FIGURE']);
const ALLOWED_FONT_SIZES = new Set(['10', '12', '14', '16', '18', '20', '24', '28', '32', '36', '48', '72']);
...
const sanitizeNode = (node, document) => {
  ...
  if (node.getAttribute('data-rich-editor-media-insertion-marker') === 'true') {
    node.remove();
    return;
  }
  ...
  const preservedIsGallery = node.dataset?.gallery === 'true' || node.getAttribute('data-gallery') === 'true';
  const preservedIsImage = node.dataset?.image === 'true' || node.getAttribute('data-image') === 'true';
  const preservedCaption = node.dataset?.caption || node.getAttribute('data-caption') || '';
  const preservedCredit = node.dataset?.credit || node.getAttribute('data-credit') || '';
  ...
  if (tag === 'FIGURE') {
    ...
    if (preservedIsGallery) {
      node.setAttribute('data-gallery', 'true');
    } else if (preservedIsImage || node.querySelectorAll('img').length === 1) {
      node.setAttribute('data-image', 'true');
    } else {
      node.setAttribute('data-gallery', 'true');
    }

    if (preservedCaption.trim()) {
      node.setAttribute('data-caption', preservedCaption.trim());
    }

    if (preservedCredit.trim()) {
      node.setAttribute('data-credit', preservedCredit.trim());
    }
  }

  if (tag === 'FIGCAPTION' && !node.textContent?.trim()) {
    node.remove();
    return;
  }

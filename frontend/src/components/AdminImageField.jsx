...
const getImageOrientationLabel = (width, height) => {
  if (!width || !height) {
    return '';
  }
...
const AdminImageField = ({
  ...
  helperText = 'Você pode colar uma URL ou enviar uma imagem do computador.',
  positionValue = '',
  onPositionChange,
    ...
    <div className="space-y-3">
      ...
      {hasPositionControls ? (
        <div className="rounded-[24px] border border-charcoal/10 bg-white/60 p-4 space-y-3">
          ...
          <div>
            ...
            <p className="mt-2 text-xs text-stone">
              Use os atalhos para enquadrar mais rápido. Retratos costumam funcionar bem com{' '}
              <strong>center 18%</strong>; paisagens com <strong>center center</strong>.
            </p>
            <p className="mt-2 text-xs text-stone">
              Para capa/fundo da matéria, prefira imagem horizontal com mínimo de{' '}
              <strong>1600x900</strong> e ideal de <strong>1920x1080</strong>. Imagens verticais
              ou quadradas são aceitas, mas podem usar enquadramento adaptativo no site.
            </p>
          </div>
        </div>
      ) : (
        <div className="rounded-[20px] border border-dashed border-charcoal/10 bg-white/35 px-4 py-3 text-xs text-stone">
          ...
          paisagens com <strong>center center</strong>. Nas telas que aceitam posição de imagem,
          ...
          <SafeImage
            ...
            onLoad={(event) => {
              ...
              setImageInfo({
                ...
                orientation: getImageOrientationLabel(naturalWidth, naturalHeight)
              });
            }}
          />
          {imageInfo && (
            <div className="mt-3 rounded-2xl border border-charcoal/10 bg-white/70 px-4 py-3 text-xs text-stone">
              <strong className="text-charcoal">{imageInfo.orientation}</strong>
              {' '}• {imageInfo.width}x{imageInfo.height}px
              {hasPositionControls && imageInfo.orientation !== 'Horizontal' ? (
                <span>
                  {' '}• Para capa grande, esta imagem pode aparecer com fundo adaptativo para
                  preservar nitidez e evitar cortes fortes.
                </span>
              ) : null}
            </div>
          )}

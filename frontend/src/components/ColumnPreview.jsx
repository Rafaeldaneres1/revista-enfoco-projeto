...
const ColumnPreview = ({ column, previewLabel = 'Prévia da coluna' }) => {
      ...
      <div className={`relative z-10 px-6 md:px-10 ${column?.featured_image ? '-mt-16 md:-mt-24' : 'pt-10'} pb-10`}>
        <div className="bg-white md:max-w-3xl mx-auto rounded-[24px] md:rounded-[28px] px-6 md:px-10 pt-8 md:pt-10 pb-8 shadow-premium-sm border border-charcoal/6">
          ...
          <p className="text-lg md:text-xl text-stone leading-relaxed mb-8">
            {column?.excerpt || 'O resumo da coluna aparecerá aqui para revisar antes de publicar.'}
          </p>
          ...
          <RichTextContent
            ...
            html={
              renderedContent ||
              '<p>Comece a escrever o conteúdo da coluna para ver a prévia editorial aqui.</p>'
            }
          />
            ...
            <div className="flex items-start gap-6">
              {column?.author_image ? (
                <SafeImage
                  src={column.author_image}
                  alt={column.author_name || 'Colunista'}
                  className="w-20 h-20 rounded-full object-cover object-[center_30%] shadow-md flex-shrink-0"
                  cloudinaryVariant="avatar"
                  sizes="160px"
                />
              ) : (
              ...
              <div>
                ...
                <p className="text-sm text-stone leading-relaxed">
                  {column?.author_bio || 'A bio do colunista aparecerá aqui quando preenchida.'}
                </p>
              </div>
            </div>
          </div>

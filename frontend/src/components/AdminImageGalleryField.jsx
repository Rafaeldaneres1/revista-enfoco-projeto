...
const AdminImageGalleryField = ({
  ...
  const validateFiles = (files) => {
    ...
    for (const file of files) {
      ...
      if (file.size > MAX_FILE_SIZE_BYTES) {
        return `Cada imagem deve ter no máximo ${MAX_FILE_SIZE_MB} MB.`;
      }
    }
  ...
  const handleMultipleFilesChange = async (event) => {
    ...
    try {
      ...
      setSuccessMessage(
        `${uploadedUrls.length} ${uploadedUrls.length === 1 ? itemLabel.toLowerCase() : `${itemLabel.toLowerCase()}s`} adicionada${uploadedUrls.length === 1 ? '' : 's'} com sucesso. Clique em salvar para gravar na edição.`
      );
    } catch (uploadError) {
      ...
      if (uploadError.code === 'ECONNABORTED') {
        ...
        setError(uploadError?.response?.data?.detail || 'Não foi possível enviar as imagens selecionadas.');
      }
    } finally {
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-xs text-stone mt-1">
            Selecione várias imagens de uma vez. Elas aparecem na ordem em que forem adicionadas.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          >
            {uploading ? 'Enviando imagens...' : 'Enviar várias fotos'}
          </button>
          <button
            type="button"
            onClick={addImage}
      ...
      {normalizedValues.length > 0 ? (
        <div className="flex items-center justify-between gap-3 flex-wrap rounded-[20px] border border-charcoal/10 bg-white/55 px-4 py-3 text-sm text-charcoal">
          ...
          <span className="text-xs text-stone">
            Reordene com as setas. A numeração é atualizada automaticamente.
          </span>
        </div>
      ) : null}
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-stone">
                        {isStored ? `Posição ${pageNumber} na lista` : item.status}
                      </p>
                    </div>
